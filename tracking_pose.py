...
def main():
    ...
    task = Task.init(
        project_name="odnn15",
        task_name=f"pose-tracking_{MODEL_TAG}_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
    ...
    SHOW_BOUNDING_BOXES = False  # bounding boxes
    SHOW_POSE = True           # ключевые точки и скелет
    STROKE_WIDTH = 2            # ширина линии
    
    # каждый объект - свой цвет
    def get_random_color(obj_id):
        import random
        random.seed(obj_id)  # сохранияем цвет для всего объекта
        return (random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))
    
    video_path = 'data/input/people_1.mp4'
    cap = cv2.VideoCapture(video_path)
    output_path = f'data/output/out-people-{MODEL_TAG}-conf-05.mp4'
        ...
        if results[0].boxes is not None and results[0].boxes.id is not None:
            for i, box in enumerate(results[0].boxes):
                ...
                if int(box.cls[0]) in allowed_indices and conf > 0.5:
                    ...
                    obj_id = int(results[0].boxes.id[i])  # ID объекта
